...
print " ON STARDATE ".($T0+$MaxNumOfDays).". THIS GIVES YOU $MaxNumOfDays DAYS.\n";
print " THERE".$Ss0;
print " $TotalStarbases STARBASE".$Ss." IN THE GALAXY FOR RESUPPLYING YOUR SHIP.\n";
print " \n\n\n";
...
print "\nIT IS STARDATE ".(int($Stardate*10)*.1)."\n";
...
sub CourseControl {
	...
	for ($I=1;$I<$NoOfSteps;$I++) {
		...
		if (substr($QuadString,$position-1,2) ne "  ") {
			...
			telePrint("SECTOR $S1 , $S2 DUE TO BAD NAVIGATION.",0.5);
			last;
		}
	}
...
sub FirePhasers {
	...
	for ($I=1;$I<=3;$I++) {
		...
		if ($KlingonEnergy >0) {
			...
			telePrint("*** KLINGON DESTROYED ***",0.5);
	...
	# now it's Klingons' turn
	KlingonsAttack();
	#print "End of Phasers Attack\n";
	return 0;
}
...
sub DamageControl {
	...
	if ($ShipDocked) {
		...
		print "ESTIMATED TIME TO REPAIR: ".(0.01*int(100*$TimeToRepair))." STARDATES\n";	# reduce the number to 2 decimals
